...
const Result: React.FC<ResultProps> = ({ commonMovies }) => {
    ...
    return (
        <Flex display-name="results-flex">
            <Text>The selected actors have {commonMovies.length} movies in common! Their latest movie together was {latestMovie.title} which came out in {latestMovieYear}</Text>
        </Flex>
    )
}
